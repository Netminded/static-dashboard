    ...
    <div ref="depPanelArea" :class="['deps-viewer', isThumbnailPreview && 'deps-viewer-thumbnail', setBkColor(backgroundOption)]" :style="{ backgroundColor: customBackgroundOption }">
        ...
        <template v-for="(dep, index) in depList" :key="index">
            <template v-if="dep.added">
                <Tile ref="tileEls" :id="dep.id" :title="dep.title" :description="dep.description" :createdBy="dep.createdBy" :statusMsg="dep.statusMsg" :statusColor="dep.statusColor" 
                    :supportMsg="dep.supportMsgs[dep.statusColor]" :thirdPartyItem="dep.thirdPartyItem" :depList="depList" :isThumbnailPreview="isThumbnailPreview" />
